...
class SearchUsersNotInGroup
    implements UseCase<List<UserProfile>, SearchUsersNotInGroupParams> {
  ...
  Future<Either<Failure, List<UserProfile>>> call(
    SearchUsersNotInGroupParams params,
  ) async {
    try {
      final users = await repository.searchUsersNotInGroup(
        params.query,
        params.groupId,
      );
      return Right(users);
...
class SearchUsersNotInGroupParams extends Equatable {
  ...
  const SearchUsersNotInGroupParams({
    required this.query,
